...
wgd_test <- function(facets,out){
 out_file=as.character(out)
  ...
  for (s in samples){
    ...
    if(n.bases > 0){
      loh <- sum(as.numeric(fit$seglen[which(facets.fit$lcn == 0)]))
    ...
    if(wgd){
      cn.neutral <- sum(as.numeric(fit$seglen[which(facets.fit$tcn == 4)]))
      cn.neutral.v2 <- sum(as.numeric(fit$seglen[which((facets.fit$tcn == 4) | (facets.fit$tcn == 2) | (is.na(facets.fit$tcn)))]))
      f_altered <- 1 - (cn.neutral / n.bases)
      f_altered_v2 <- 1 - (cn.neutral.v2 / n.bases)
    } else {
      cn.neutral <- sum(as.numeric(fit$seglen[which(facets.fit$tcn == 2)]))
