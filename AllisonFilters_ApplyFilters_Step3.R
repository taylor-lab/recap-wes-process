...
for(j in 89:110)
{
 ...
 fill <- fill[!duplicated(fill$Ttag),]
  rownames(fill) <- fill$Ttag <- paste(sapply(strsplit(as.character(fill$Tumor_Sample_Barcode),"[.]"),function(x){x[1]}),fill$Chromosome,fill$Start_Position,fill$Reference_Allele,fill$Tumor_Seq_Allele1,sep=";")
  ...
  rownames(Nfill) <- Nfill$Ntag <- paste(sapply(strsplit(as.character(Nfill$Tumor_Sample_Barcode),"[.]"),function(x){x[1]}),Nfill$Chromosome,Nfill$Start_Position,Nfill$Reference_Allele,Nfill$Tumor_Seq_Allele1,sep=";")
  ...
  maf_ = distinct(maf0) %>% mutate(var_tag = str_c(Tumor_Sample_Barcode,':',Chromosome,':',Start_Position,':',End_Position,':',Reference_Allele,':',Tumor_Seq_Allele2))
  dim(maf_)
  ...
  table(maf_$FILTER)
  dim(maf_)
  #write.table(maf_,"/ifs/res/taylorlab/chavans/roslin_2.4_deliveries/cs_deliveries/final/Proj_07058_G.muts.ccs_tagged.maf",quote=FALSE,row.names = FALSE,sep="\t",append=FALSE)
